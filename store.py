...
def list_jobs(
    ...
    within a score tier. `score` is NULL until run.py's scoring pass (score.py)
    fills it in for a given job — filtering on min_score naturally excludes
    anything not scored yet."""
    clauses = []
    params = []
    if source:
